...
**AI Engineer.** I build AI systems that run real business operations.

A few years ago I was memorizing Biology chapters on the Pre-Medical track. Then I spent 5 to 6 months researching where the world was heading, switched to a BS in Artificial Intelligence with zero programming background, caught up fast (TA for a 10-month AI certification, co-founder of a university AI club), and landed my first automation engineering role in 7th semester, before graduating.
...
`n8n` `Make.com` `Python` `Next.js` `Airtable` `OpenAI` `Claude` `Gemini` `CrewAI` `VAPI` `ElevenLabs` `FastAPI` `Docker` `Linux` `self-hosted infrastructure`
